...
- **1.2.0 기능 구현 완료** (공유 + 읽기전용 뷰어 + 둘러보기 + 내 공유물 관리). 상세 내용은 [`docs/roadmap.md`](docs/roadmap.md)에 통합됨.
  ...
  - ✅ 심사 문서: `privacy-policy.md` + `app-review-notes` 1.2.1 기준 갱신
...
- **1.2.1 — 배포 준비 완료**. `app.json` 버전 1.2.1, 종목 검색 API 변경 대응, 한국 종목 현재가 fallback, 스크린샷 3장 구성 반영.
  - ✅ 처리방침(`privacy-policy.md`) 공유 전송 데이터 포함 버전으로 교체 → **GitHub Pages 게시 필요**
  - ✅ 심사 노트(`app-review-notes.md`) 1.2.1 기준 갱신 + App Privacy 가이드(User ID/User Content 수집 신고)
...
| # | 항목 | 산출물 / 메모 |
|---|---|---|
| 1 | **스크린샷 자동 캡처 하니스** | `scripts/screenshots.sh`, `src/utils/screenshot.js`, `screenshots/01~03-*.png` (1320×2868, 6.9") |
...
- **버전 관리**: marketing 버전은 `app.json`의 `version`(현재 `1.2.1`)에서 직접 올림. 빌드 번호는 EAS가 자동 증가.
