# 변경 이력 (Changelog)

이 프로젝트의 주요 변경사항을 기록합니다.
버전은 `app.json`의 marketing 버전 기준이며, 빌드 번호는 EAS가 자동 증가합니다.

---

## [1.2.1] — 2026-06-27 (배포 준비)

### 수정 (Fixed)
- **종목 검색 API 변경 대응** — 네이버 주식 검색 API가 `target=stock,etf`를 더 이상 허용하지 않아
  iOS 시뮬레이터/실기기에서 종목 검색이 실패하던 문제를 수정했습니다. 검색 파라미터를 현재 허용값인
  `target=stock`으로 변경했습니다.
- **한국 종목 현재가 보강** — Yahoo Finance가 한국 심볼(`.KS`, `.KQ`) 현재가 요청을 429로 차단하는 경우,
  네이버 현재가 API로 fallback 하도록 수정했습니다.

### 문서/스크린샷 (Docs)
- App Store 스크린샷 구성을 개별계좌 / 종합계좌 / 다른 사람 포트폴리오 구경하기 3장으로 정리했습니다.

---

## [1.2.0] — 2026-06-19 (배포 준비)

> **첫 서버 전송 릴리스** — 선택적 포트폴리오 공유와 둘러보기 탭을 도입했습니다.
> 공유는 **비중(%)만** Supabase로 전송하며 금액·보유수량·현재가는 전송하지 않습니다.
> 공유 기능 사용 시에만 데이터가 수집되므로 처리방침·App Privacy 설문을 갱신했습니다.

### 추가 (Added)
- **포트폴리오 공유** — 통합 화면에서 비중만 추출해 게시. 별명+비밀번호 인증
  (이메일/소셜 없음, 비밀번호는 서버에서 bcrypt 해시). 공유 코드로 읽기전용 열람.
- **둘러보기 탭** — 공개 포트폴리오 6개월 수익률 **Top 5**, **별명/종목 검색**,
  **더보기(+10)**. 공유 기본값은 "공개 + 순위 등재"(기존 `backtest.js`로 수익률 계산·제출).
- **내 공유물 관리** — 별명+비밀번호로 로그인해 제목/공개범위 **수정**·**삭제**.
- **UGC 안전장치 (Guideline 1.2)** — EULA 동의, 신고(누적 시 자동 숨김), 작성자 차단,
  별명/제목 비속어 필터.
- **스플래시 화면** — 앱 아이콘 중앙 + 흰 배경(`expo-splash-screen`).

### 개선/정리 (Changed)
- 통합 탭은 내 포트폴리오 기능만, 소셜(순위·검색·코드로 보기·내 공유물)은 둘러보기 탭으로 분리.
- 존재하지 않던 `web.favicon` 참조 제거(app.json).

### 문서 (Docs)
- `docs/privacy-policy.md` 를 공유 전송 데이터 포함 버전으로 교체(라이브 게시 대상).
- `docs/app-review-notes.md` 를 1.2.0(공유/UGC/Supabase) 기준으로 갱신, App Privacy 가이드 포함.

### 보안/프라이버시
- RLS로 "공개분만 읽기, 본인만 쓰기" 강제. service_role 키는 클라이언트 미포함.
- 비중만 공유, 이메일 미수집. App Privacy: User ID + User Content 수집 신고(추적 아님).

---

## [1.1.0] — 2026-06-18 (App Store 배포 완료)

> 6개월 보유 백테스트와 N일 주기 리밸런싱 백테스트를 포함하고,
> 실기기 테스트에서 발견한 UX 문제와 버그를 수정한 버전입니다. App Store 심사를
> 통과해 배포 완료되었습니다.

### 수정 (Fixed)
- **한글 종목명 검색이 안 되던 문제** — `삼성전자` 등 한글 검색이 "결과 없음"으로
  실패했다. 원인은 Yahoo 검색 엔드포인트가 브라우저 User-Agent 없는 요청을 429로
  차단하는 것(영문 `AAPL`도 동일하게 실패 — 한글 문제가 아님). 검색을 **네이버 주식
  통합검색**(`m.stock.naver.com/front-api/search`)으로 전환해, 한국주식(한글명)·
  미국주식(티커/한글명)·ETF를 모두 찾고 종목코드를 Yahoo 심볼(`.KS`/`.KQ`/티커)로
  매핑한다. 자동완성은 이름 앞부분만 매칭해 ETF를 키워드(예: "반도체", "2차전지")로
  못 찾던 문제가 있어, 부분·키워드 매칭이 되는 통합검색을 사용한다. 시세·과거시세는
  기존 Yahoo chart 엔드포인트(브라우저 User-Agent 적용)로 가져온다.
  (`src/services/stockApi.js` — `searchStocks`, `toYahooSymbol`)
- **Yahoo 호출에 브라우저 User-Agent 누락** — 시세/과거시세 호출도 UA가 없어 429로
  차단됐다. 공통 헤더(`REQUEST_HEADERS`)로 모든 Yahoo 호출에 User-Agent를 적용. 검색
  실패를 "검색 결과가 없습니다"로 오인 표시하던 UX도 수정(`StockSearchModal`).
- **숫자 입력 후 키보드를 내릴 수 없던 문제** — `decimal-pad` 입력칸(현재가·보유수량·비중,
  총 금액)은 iOS에서 "완료/Return" 키가 없어 키보드가 갇혔고, 항목 편집 모달에서는
  키보드가 저장 버튼까지 가렸습니다.
  - 항목 편집 모달(`ItemEditorModal`): 입력칸 바깥(배경·빈 공간)을 탭하면 키보드가
    닫히고, `KeyboardAvoidingView`로 시트를 밀어올려 저장 버튼이 가려지지 않도록 함.
    (iOS `InputAccessoryView`는 RN `Modal` 내부에서 렌더되지 않아 모달용으로는
    사용 불가 — 해당 한계를 우회.)
  - 계좌 화면 총 금액칸(`AccountScreen`): 숫자패드 위에 "완료" 막대
    (`InputAccessoryView`) 추가. (모달 밖이라 정상 동작)
- **항목 수정이 백테스트에 반영되지 않던 문제** — 6개월 백테스트 결과 캐시가
  `activeAccount.id`가 바뀔 때만 무효화되어, 같은 계좌에서 비중·종목을 수정/추가/삭제해도
  옛 결과가 그대로 표시됐습니다. 캐시 무효화 기준을 항목 입력값 시그니처
  (`심볼:종목명:목표비중`)로 변경해, 수정 시 다시 계산되도록 함. (`AccountScreen`)
  - 통합(Consolidated) 화면은 이미 `holdings` 기준으로 무효화하고 있어 정상 동작 확인.

### 개선 (Changed)
- **종목명을 한글로 표시** — 검색 결과를 선택할 때 Yahoo 의 영문 이름
  (예: "Samsung Electronics")으로 덮어쓰던 것을, 네이버 검색의 **한글명**
  (예: "삼성전자")을 유지하도록 변경. Yahoo 에서는 가격·통화만 가져온다.
  (`StockSearchModal.handleSelect`) ※ 이미 저장된 기존 항목은 다시 검색·선택해야
  한글명으로 갱신됨.
- **오버레이 모달 배경 탭으로 닫기** — 화면 위로 떠오르는 바텀시트 모달의 어두운 배경을
  누르면 닫히도록 함(iOS 표준 동작). 적용: 백테스트 결과(`BacktestModal`),
  종목 검색(`StockSearchModal`), 항목 추가/편집(`ItemEditorModal`).
  시트 본체를 누르면 닫히지 않으며, 항목 편집 모달은 배경 탭 시 키보드를 먼저 닫습니다.

### 문서 (Docs)
- `docs/privacy-policy.md`에 Jekyll front matter 추가 — GitHub Pages가 Markdown을
  HTML로 렌더링하도록 해, App Store에 등록할 처리방침 URL
  (`/privacy-policy.html`)이 정상적으로 열리도록 함.

---

## [1.0.0] — 첫 제출 (App Store 심사)

- N일 주기 리밸런싱 백테스트, 6개월 buy-and-hold 백테스트
- 계좌별 포트폴리오 구성·목표 비중·권장 매수 주수 계산
- 모든 계좌 통합(Consolidated) 종목별/그룹별 보기
- Yahoo Finance 공개 API로 종목 검색·현재가·과거 시세 조회
- JSON 내보내기, 스크린샷 자동 캡처 하니스, EAS 빌드/배포 설정
